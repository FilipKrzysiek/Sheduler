...
int main(int argc, char *argv[])
{
    ...
    scheduler.setEndWorkTimeAfter(1min);
    ...
    cout << "Info from main thread" << endl;

    this_thread::sleep_for(15s);

    cout << "Main thread. Stoping scheduler!" << endl;

    scheduler.stop();

    cout << "Main thread. Scheduler stoped!" << endl;

    this_thread::sleep_for(15s);

    cout << "Main thread. Started scheduler again and wait until end." << endl;

    scheduler.start();

    cout << "Main thread. Scheduler working." << endl;

    scheduler.wait();

    cout << "Main thread. Scheduler end working." << endl;
